...
const INVENTORY_PROPERTIES: PropertyDefinition[] = [
  ...
  { id: 'stock_quantity', label: 'Estoque', type: 'number', defaultVisible: true },
  ...
  { id: 'min_stock', label: 'Estoque Mínimo', type: 'number', defaultVisible: false },
];
...
export default function Inventory() {
  ...
  const [visibleProperties, setVisibleProperties] = useState<string[]>(
    prefs.properties || INVENTORY_PROPERTIES.filter(p => p.defaultVisible).map(p => p.id)
  );
  ...
  useEffect(() => {
    saveViewPreference('inventory', { mode: viewMode, properties: visibleProperties });
  ...
  const finishedProducts = filteredProducts.filter(p => p.category !== 'insumo');
  ...
  function renderProductCard(product: any) {
    return (
      <Card key={product.id} className="p-4 hover:shadow-lg cursor-pointer transition-smooth" onClick={() => navigate(`/products/${product.id}`)}>
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between mt-1">
          <Badge variant={isLowStock(product) ? 'destructive' : 'secondary'} className="text-xs">
          ...
          {viewMode === 'list' && (
            <div className="space-y-2">
              {finishedProducts.map(product => (
                <Card key={product.id} className="p-4 hover:shadow-md cursor-pointer transition-smooth" onClick={() => navigate(`/products/${product.id}`)}>
                  <div className="flex items-center gap-4">
                    ...
                    <p className="font-semibold">R$ {Number(product.unit_price || 0).toFixed(2)}</p>
                  </div>
                </Card>
              ))}
            </div>
                ...
                <TableHeader>
                  <TableRow>
                    {visibleProperties.map(propId => {
                      const prop = INVENTORY_PROPERTIES.find(p => p.id === propId);
                ...
                <TableBody>
                  {finishedProducts.map(product => (
                    <TableRow key={product.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/products/${product.id}`)}>
                      {visibleProperties.includes('name') && <TableCell className="font-medium">{product.name}</TableCell>}
                      {visibleProperties.includes('sku') && <TableCell>{product.sku || '-'}</TableCell>}
                      ...
                      {visibleProperties.includes('category') && <TableCell>{product.category}</TableCell>}
                      <TableCell>
              ...
              {supplies.map(product => (
                <Card key={product.id} className="p-4 hover:shadow-md cursor-pointer transition-smooth" onClick={() => navigate(`/products/${product.id}`)}>
                  <div className="flex items-center gap-4">
                    ...
                    <p className="font-semibold">R$ {Number(product.cost_price || 0).toFixed(2)}</p>
                  </div>
                </Card>
              ))}
            </div>
                  ...
                  {supplies.map(product => (
                    <TableRow key={product.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/products/${product.id}`)}>
                      ...
                      {visibleProperties.includes('cost_price') && (
                        <TableCell>R$ {Number(product.cost_price || 0).toFixed(2)}</TableCell>
                      )}
                      {visibleProperties.includes('category') && <TableCell>{product.category}</TableCell>}
                      <TableCell>
                        <Button variant="outline" size="sm" onClick={(e) => {
