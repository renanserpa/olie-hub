...
export async function runConfigsDiagnostics(): Promise<DiagnosticCheck[]> {
  ...
  const tablesToCheck = [
    ...
    { table: 'config_customization_components', name: 'Componentes de Personalização' },
  ...
  for (const { table, name } of tablesToCheck) {
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST204' || error.message.includes('does not exist') || (error.message.includes('relation') && error.message.includes('does not exist'))) {
          checks.push({
            name,
            status: 'error',
