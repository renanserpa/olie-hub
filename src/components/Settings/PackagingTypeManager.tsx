...
export function PackagingTypeManager() {
  ...
  const loadTypes = useCallback(async () => {
    ...
    const { data, error: fetchError } = await listConfigs<PackagingType>('config_packaging_types', {
      search: searchTerm,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const payload = {
      ...
      dimensions_json: values.dimensions_json && values.dimensions_json.trim().length > 0
    ...
    const action = editing
      ? await updateConfig<PackagingType>('config_packaging_types', editing.id, payload)
    ...
    toast.success(editing ? 'Tipo atualizado com sucesso' : 'Tipo criado com sucesso');
    setDialogOpen(false);
    setEditing(null);
    loadTypes();
  };
  ...
  const handleToggleActive = async (type: PackagingType) => {
    ...
    if (type.is_active && !confirm('Tem certeza que deseja arquivar este tipo de embalagem?')) {
      return;
    }

    const { error: toggleError } = await updateConfig<PackagingType>('config_packaging_types', type.id, {
    ...
    <div className="space-y-4">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Estruture embalagens padrão com dimensões, materiais e limites de peso
          </p>
        </div>
      ...
      {(!isAdmin && !checkingAdmin) && (
        <Alert variant="default" className="border-dashed">
          ...
          <AlertDescription>
            Você não possui permissões de administrador. É possível visualizar as embalagens, mas não criar ou editar.
          </AlertDescription>
        </Alert>
      )}
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="hidden md:table-cell">Limite (g)</TableHead>
              ...
              {types.map((type) => (
                <TableRow key={type.id}>
                  <TableCell>
                    ...
                    <div className="text-xs text-muted-foreground md:hidden">Código: {type.codigo}</div>
                    <div className="text-xs text-muted-foreground">
                      Dimensões: {JSON.stringify(type.dimensions_json ?? {}, null, 0)}
                    </div>
                  </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>{editing ? 'Editar tipo' : 'Novo tipo de embalagem'}</DialogTitle>
          </DialogHeader>
