...
export function BondTypeManager() {
  ...
  const loadBondTypes = useCallback(async () => {
    ...
    const { data, error: fetchError } = await listConfigs<BondType>(
      "config_bond_types",
      {
        search: searchTerm,
    ...
    if (dialogOpen) {
      if (editing) {
        form.reset({
          ...
          payroll_effects_json: JSON.stringify(
            editing.payroll_effects_json ?? {},
            null,
            2,
          ),
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const payload = {
      ...
      payroll_effects_json:
        values.payroll_effects_json &&
        values.payroll_effects_json.trim().length > 0
          ? (parseJsonOrNull<Record<string, unknown>>(
              values.payroll_effects_json,
            ) ?? {})
          : {},
    ...
    toast.success(
      editing ? "Vínculo atualizado com sucesso" : "Vínculo criado com sucesso",
    );
    setDialogOpen(false);
    setEditing(null);
    loadBondTypes();
  ...
  const handleToggleActive = async (bondType: BondType) => {
    ...
    if (
      bondType.is_active &&
      !confirm("Tem certeza que deseja arquivar este vínculo?")
    ) {
      return;
    }

    const { error: toggleError } = await updateConfig<BondType>(
      "config_bond_types",
      bondType.id,
      {
    ...
    toast.success(
      bondType.is_active ? "Vínculo arquivado" : "Vínculo reativado",
    );
    loadBondTypes();
  };
    ...
    <div className="space-y-4">
      ...
      {!isAdmin && !checkingAdmin && (
        <Alert variant="default" className="border-dashed">
          ...
          <AlertDescription>
            ...
            os vínculos, mas não criar ou editar.
          </AlertDescription>
        </Alert>
      )}
                ...
                <TableRow key={bondType.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={bondType.is_active ? "default" : "outline"}
                      >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {editing ? "Editar vínculo" : "Novo tipo de vínculo"}
            </DialogTitle>
          </DialogHeader>
